...
export function WithAssistantView({ mode }: { mode: Mode }) {
  ...
  return (
    <div className="flex h-full overflow-hidden">
      {/* Outline / jump nav — flat, borderless */}
      <div className="w-[200px] shrink-0 h-full flex flex-col">
        <div className="shrink-0 h-[56px] flex items-center px-[16px]">
          <span className="t-title-md text-[var(--foreground-primary,#1a1a1a)] flex-1" style={{ fontFeatureSettings: "'ss07'" }}>{templateName}</span>
        </div>
        ...
        {/* Save at the bottom of the master list */}
        <div className="shrink-0 p-[12px]">
          <Button variant="primary" size="medium" prefix={<Icon name="save" size={18} />} className="w-full">
            Save Template
          </Button>
        </div>
      </div>

      {/* Center pane — Settings / Preview tabs */}
      <div className="flex-1 min-w-0 flex flex-col border-l border-[var(--shape-outline,rgba(0,0,0,0.1))]">
        ...
        {centerTab === "settings" ? (
          <div ref={scrollRef} className="flex-1 min-h-0 overflow-y-auto">
            <div className="px-[24px] pb-[24px] flex flex-col gap-[32px] max-w-[720px]">
              {sections.map((sec) => (
