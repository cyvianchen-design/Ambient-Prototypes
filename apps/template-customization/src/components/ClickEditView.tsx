...
// ── Include data from (click-to-edit chips) ───────────────────────────────────
...
const TIME_RANGE_ABBREV: Record<string, string> = {
  "Since last visit": "since last visit", "Last 24 hrs": "24h",
  "Last 7 days": "7d", "Last 30 days": "30d", "All time": "all time",
};
...
function IncludeDataField({ mode }: { mode: Mode }) {
  ...
  function openNoteModal(noteType: string, existingIndex?: number) {
    setOpen(false);
    if (existingIndex !== undefined) {
      const existing = selected[existingIndex] as NoteItem;
      setPendingFrom(existing.from);
      setPendingWithin(existing.within);
      setPendingSections(new Set(existing.sections));
  ...
  function openDataModal(type: string, existingIndex?: number) {
    setOpen(false);
    if (existingIndex !== undefined) {
      const existing = selected[existingIndex] as DataItem;
      setPendingTimeRange(existing.timeRange);
      setEditingIndex(existingIndex);
    } else {
  ...
  function confirmModal() {
    if (!modalNote) return;
    const newItem: NoteItem = { kind: "note", noteType: modalNote, from: pendingFrom, within: pendingWithin, sections: Array.from(pendingSections) };
    if (editingIndex !== null) {
      setSelected((p) => p.map((item, i) => i === editingIndex ? newItem : item));
    } else {
      setSelected((p) => [...p, newItem]);
    }
    setModalNote(null);
    setEditingIndex(null);
  }
  ...
  function confirmDataModal() {
    if (!modalData) return;
    const newItem: DataItem = { kind: "data", type: modalData, timeRange: pendingTimeRange };
    if (editingIndex !== null) {
      setSelected((p) => p.map((item, i) => i === editingIndex ? newItem : item));
    } else {
      setSelected((p) => [...p, newItem]);
  ...
  function handleChipClick(index: number) {
    ...
    if (item.kind === "note") openNoteModal(item.noteType, index);
    else openDataModal(item.type, index);
  }
    ...
    <>
      <div className="flex flex-wrap gap-[6px] items-center">
        ...
        {selected.map((item, i) => (
          <Chip
            key={i}
            label={includeChipLabel(item)}
            color="neutral"
            size="XS"
            onClick={() => handleChipClick(i)}
            onDismiss={() => setSelected((p) => p.filter((_, j) => j !== i))}
          />
        ))}
      ...
      {modalNote && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center">
          <div className="absolute inset-0 bg-black/20" onClick={() => { setModalNote(null); setEditingIndex(null); }} />
          <div className="relative z-[160] bg-white rounded-[16px] shadow-[0_8px_32px_rgba(0,0,0,0.16)] w-[440px] max-h-[80vh] flex flex-col overflow-hidden">
            <div className="shrink-0 px-[24px] pt-[20px] pb-[16px]">
              <span className="t-title-lg text-[var(--foreground-primary,#1a1a1a)]">
                {editingIndex !== null ? `Edit ${modalNote}` : `Add from ${modalNote}`}
              </span>
            </div>
            ...
            <div className="shrink-0 px-[24px] py-[16px] flex justify-end gap-[8px]">
              <Button variant="secondary" size="medium" onClick={() => { setModalNote(null); setEditingIndex(null); }}>Cancel</Button>
              <Button variant="primary" size="medium" onClick={confirmModal}>
      ...
      {modalData && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center">
          <div className="absolute inset-0 bg-black/20" onClick={() => { setModalData(null); setEditingIndex(null); }} />
          <div className="relative z-[160] bg-white rounded-[16px] shadow-[0_8px_32px_rgba(0,0,0,0.16)] w-[360px] flex flex-col overflow-hidden">
            <div className="shrink-0 px-[24px] pt-[20px] pb-[16px]">
              <span className="t-title-lg text-[var(--foreground-primary,#1a1a1a)]">
                {editingIndex !== null ? `Edit ${modalData}` : `Add ${modalData}`}
              </span>
            </div>
            ...
            <div className="shrink-0 px-[24px] py-[16px] flex justify-end gap-[8px]">
              <Button variant="secondary" size="medium" onClick={() => { setModalData(null); setEditingIndex(null); }}>Cancel</Button>
              <Button variant="primary" size="medium" onClick={confirmDataModal}>
                {editingIndex !== null ? "Update" : "Add"}
              </Button>
            </div>
          </div>
...
export function ClickEditView({ mode }: { mode: Mode }) {
