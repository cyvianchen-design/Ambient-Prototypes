...
// ── Include data from (inline expand) ────────────────────────────────────────
...
function IncludeDataField() {
  ...
  function confirmNote(noteType: string) {
    setSelected((p) => [...p, { kind: "note", noteType, from: pendingFrom, within: pendingWithin, sections: Array.from(pendingSections) }]);
    setExpandedNote(null);
    setOpen(false);
  }
      ...
      <span className="shrink-0 t-title-sm text-[var(--foreground-secondary,#666)]">Include data from</span>
        ...
        {(availableNotes.length > 0 || availableData.length > 0) && (
          <div ref={containerRef} className="relative">
            ...
            {open && (
              <div className="absolute top-full left-0 mt-[4px] z-50 bg-white rounded-[12px] shadow-[0_4px_20px_rgba(0,0,0,0.12)] border border-[rgba(0,0,0,0.08)] w-[320px] overflow-hidden">
                {availableNotes.length > 0 && (
                  <div className="px-[6px] pt-[6px]">
                    <MenuHeader>Notes</MenuHeader>
                    {availableNotes.map((noteType) => (
                      <div key={noteType}>
                        <MenuItem
                          label={noteType}
                          trailing={<Icon name={expandedNote === noteType ? "keyboard_arrow_up" : "chevron_right"} size={16} />}
                          onClick={() => openNote(noteType)}
                        />
                          ...
                          <div className="mx-[6px] mb-[4px] rounded-[8px] bg-[var(--surface-1,#f7f7f7)] p-[12px] flex flex-col gap-[10px]">
                            ...
                            <div className="flex gap-[8px]">
                              <div className="flex flex-col gap-[4px] flex-1 min-w-0">
                                ...
                                <SelectDropdown value={pendingFrom} options={FROM_OPTIONS} onChange={setPendingFrom} width="w-full" />
                              </div>
                              <div className="flex flex-col gap-[4px] flex-1 min-w-0">
                                <span className="t-body-xs text-[var(--foreground-secondary,#666)]">Within</span>
                            ...
                            <div className="flex justify-end">
                              <Button variant="primary" size="small" onClick={() => confirmNote(noteType)}>Add</Button>
                            </div>
                          </div>
                        )}
                      </div>
...
export function InlineExpandView({ mode }: { mode: Mode }) {
