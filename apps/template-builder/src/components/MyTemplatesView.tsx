...
function FormatChip({ label }: { label: string }) {
  return (
    <span className="bg-[#f2f2f2] rounded-[6px] px-[8px] py-[4.5px] text-[12px] text-[#1a1a1a] shrink-0 leading-[1.2]" style={{ fontFamily: "Lato" }}>
      {label}
    </span>
  );
}
...
function MacrosSection({ label, initialCount, onDelete }: { label: string; initialCount: number; onDelete: () => void }) {
  const [open, setOpen] = useState(false);
  ...
  function addMacro() {
    setMacros((prev) => [...makeMacros(prev.length + 1)]);
    setOpen(true);
  }

  // No macros attached — offer the add action alongside delete, no container.
  if (macros.length === 0) {
    return (
    ...
    <div className="bg-white border border-[rgba(0,0,0,0.1)] rounded-[6px] w-full">
      ...
      {open && (
        <div className="flex flex-col gap-[4px] px-[8px] pb-[8px]">
          ...
          <Button variant="tertiary" size="small" prefix={<Icon name="add" size={14} />} className="self-start mt-[4px]" onClick={addMacro}>
            Add Macro
          </Button>
        </div>
      )}
...
function MySubsectionRow({ sub, isDragOver, onChange, onDelete, onDragStart, onDragOver, onDrop }: RowProps) {
  const [expanded, setExpanded] = useState(sub.id === "summary" || sub.id === "pmh");
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex gap-[4px] items-center pl-[4px] pr-[4px] pt-[0px]">
        {/* Drag handle */}
      ...
      {expanded && (
        <div className="flex flex-col gap-[12px] pb-[8px] pl-[24px] pr-[8px] pt-[8px]">
          <TextArea
            ...
            value={sub.templateInstruction}
            onChange={(v) => update({ templateInstruction: v })}
            rows={4}
          />
          ...
          <MacrosSection label="Site-Macros" initialCount={sub.macroCount} onDelete={onDelete} />
        </div>
      )}
    </div>
  );
...
// ── Grouped section with drag-and-drop ───────────────────────────────────────
...
function GroupedSection({ section, onChange }: GroupedSectionProps) {
  ...
  return (
    <div className="border border-[rgba(0,0,0,0.1)] rounded-[10px] px-[4px] pt-[4px] pb-[8px] w-full flex flex-col gap-[8px]">
      <div className="h-[36px] flex items-center px-[4px]">
        <span className="text-[13px] font-bold tracking-[0.13px] text-[#1a1a1a] px-[8px] py-[4px]" style={{ fontFamily: "Lato", fontFeatureSettings: "'ss07'" }}>
          {section.name}
        </span>
      </div>
      ...
      <div className="flex flex-col gap-[4px] px-[24px]">
        ...
        <Button variant="tertiary" size="small" prefix={<Icon name="add" size={14} />} className="self-start">
        ...
        <div className="px-[20px] pt-[32px] pb-[24px] flex flex-col gap-[24px] items-center">
          ...
          <div className="w-full max-w-[600px] flex flex-col gap-[8px]">
            {sections.map((section, idx) => (
              <GroupedSection key={section.id} section={section} onChange={(updated) => updateSection(idx, updated)} />
            ))}
          </div>
        </div>
      </div>
