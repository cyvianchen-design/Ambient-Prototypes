...
import { Button, ButtonGroup, Checkbox, Chip, Icon, IconButton, Menu, MenuItem, Switch, TextArea } from "@ds/ui";
...
function IncludeDataField({ initialSelected = [] }: { initialSelected?: string[] }) {
  ...
  const available = DATA_SOURCES.filter((s) => !selected.includes(s));

  return (
    <div className="flex flex-wrap gap-[6px] items-center">
      <span className="shrink-0 t-title-sm text-[var(--foreground-secondary,#666)]">Include data from</span>
      {selected.map((s) => (
        <Chip key={s} label={s} color="neutral" size="XS" onDismiss={() => setSelected((p) => p.filter((x) => x !== s))} />
      ))}
      {available.length > 0 && (
          ...
          {open && (
            <div className="absolute top-full left-0 mt-[4px] z-50">
              <Menu className="w-[200px]">
                {available.map((s) => (
                  <MenuItem key={s} label={s} onClick={() => { setSelected((p) => [...p, s]); setOpen(false); }} />
                ))}
              </Menu>
            </div>
          )}
...
// Preview is the always-visible output on the right.
function PreviewSidebar({ sections }: { sections: TemplateSection[] }) {
  return (
...
export function PreviewFocusedView({ mode }: { mode: Mode }) {
