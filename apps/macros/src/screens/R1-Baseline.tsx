...
import { Button, Chip, Icon, TextArea } from "@ds/ui";
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="flex flex-col items-center gap-[24px] px-[20px] py-[24px]">
          ...
          <div className="w-full max-w-[600px] shrink-0">
            <h1
              className="text-[24px] font-bold leading-[1.2] text-[var(--foreground-primary,#1a1a1a)]"
          ...
          <div className="w-full max-w-[600px] flex flex-col gap-[16px] shrink-0">
            ...
            <SectionCard
              title="Subjective"
              format="Bullet by Diagnosis"
              length="Standard"
              macros={subjectiveMacros}
              totalMacros={30}
              learnExamples={10}
            />
          ...
          <div className="w-full max-w-[600px] flex flex-col gap-[16px] shrink-0 pb-[8px]">
            <div className="flex items-center justify-between">
              ...
              <button
                className="text-[13px] font-normal leading-[1.2] text-[var(--accent,#1132ee)] hover:opacity-80 transition-opacity"
            ...
            {/* Suggested rule chips */}
            <div className="flex flex-wrap gap-[8px]">
              {suggestedRules.map((rule) => (
                <Button key={rule} variant="secondary" size="small">
                  {rule}
                </Button>
              ))}
            </div>
            ...
            <TextArea
              value={customRules}
              onChange={(v) => setCustomRules(v)}
              placeholder="Enter custom formatting rules…"
              rows={5}
            />

            <p
              className="text-[12px] font-normal leading-[1.4] text-[var(--foreground-secondary,#666)]"
              style={{ fontFamily: "Lato, sans-serif" }}
            >
              You can enter custom formatting rules as if you're giving instructions to a human scribe. New rules will only apply to future scribes. If you want them to apply to existing Scribes, regenerate the Scribe to update.
            </p>
          </div>
        </div>
      </div>
